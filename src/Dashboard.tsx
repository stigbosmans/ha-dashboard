    ...
    <div className='dashboard'>
      ...
      >
        ...
        <div className='info-card'>
          ...
          <div className='time-row'>{formatTime(date)}</div>

          <div className='date-row'>{formatDate(date)}</div>

          <div className='weather-details'>
            <span className='weather-detail'>
              <i className='mdi mdi-water'></i> {Math.round(humidity)}%
            </span>
            <span className='weather-detail'>
              <i className='mdi mdi-weather-windy'></i> {weatherEntity?.attributes?.wind_speed || 5} km/h
            </span>
          </div>
        </div>
      </div>
      ...
      <div className='dashboard-bottom'>
        ...
        <div className='pagination-dots'>
          {Array.from({ length: maxPages }).map((_, index) => (
            <span key={index} className={`dot ${currentPage === index ? 'active' : ''}`} onClick={() => setCurrentPage(index)}></span>
          ))}
        </div>
      </div>
    </div>
